package com.pro.codymerebirth_hospitalsearcher

import com.google.android.gms.maps.model.LatLng

//Latinoamerica

val hospitalSucre = Hospital("Hospital Santa Barbara", 0, LatLng(-19.044638688080493, -65.26289977359298))
val hospitalCaracas = Hospital("Hospital de Clinicas Caracas", 9, LatLng(10.510392244048777, -66.89886880000002))
val hospitalBuenosAires = Hospital("Hospital Italiano de Buenos Aires", 4, LatLng(-34.60615526599895, -58.42587738679744))
val hospitalBogota = Hospital("Hospital Universitario Nacional de Colombia", 8, LatLng(4.648851154530483, -74.0958786891307))
val hospitalQuito = Hospital("Hospital Vozandes Quito", 7, LatLng(-0.17266468170644111, -78.48944501611999))
val hospitalLima = Hospital("Hospital Nacional Arzobispo Loayza", 6, LatLng(-12.049894835206626, -77.04329160256935))
val hospitalAsuncion = Hospital("Hospital Universitario Ntra. Sra. de Asunción", 1, LatLng(-25.29245087720329, -57.56199163305948))
val hospitalSantiago = Hospital("Hospital Dr. Luis Tisné Brousse", 5, LatLng(-33.50062824284426, -70.57905180031733))
val hospitalMontevideo = Hospital("Hospital Británico de Montevideo", 2, LatLng(-34.894289082319425, -56.16285437329951))
val hospitalBrasilia = Hospital("Hospital Alvorada Brasilia", 3, LatLng(-15.813129246960239, -47.91250150252596))
val hospitalGeorgetown = Hospital("Georgetown Public Hospital Corporation", 10, LatLng(6.815718116762575, -58.157052933055304))
val hospitalParamaribo = Hospital("Academic Hospital Paramaribo", 11, LatLng(5.836922274316439, -55.18308396028875))
val hospitalCayena = Hospital("Centro Hospitalario Andrée-Rosemon", 12, LatLng(4.923919567336988, -52.320488600763234))

//Europa del este


val hospitalMoscu = Hospital("Hospital clinico central", 0, LatLng(55.75007428923332, 37.38975771534646))
val hospitalHelsinki = Hospital("Surgical Hospital", 1, LatLng(60.16161953320194, 24.948867722771716))
val hospitalOslo = Hospital("Hôpital national norvégien", 2, LatLng(59.95579743610828, 10.701196370278698))
val hospitalEstocolmo = Hospital("Hospital de San Göran", 3, LatLng(59.48086427743339, 18.059729196907757))
val hospitalMinsk = Hospital("National Hospital of Belarus' Interior Ministry", 4, LatLng(54.64466763858227, 27.5106429809813))
val hospitalVarsovia = Hospital("American Clinic Warsaw", 5, LatLng(52.15515540391875, 21.077973725722103))
val hospitalBerlin = Hospital("Alexianer St. Hedwig Hospital", 6, LatLng(52.871787371307256, 13.340160586250244))
val hospitalPraga = Hospital("General University Hospital in Prague", 7, LatLng(50.07333581333177, 14.420886253960607))
val hospitalBratislava = Hospital("Hospital Bratislava Ružinov", 8, LatLng(48.47581700294231, 17.1605481190893))
val hospitalKiev = Hospital("Kyiv City Clinical Hospital", 9, LatLng(50.970627878721245, 30.41443955096359))
val hospitalBudapest = Hospital("Szent Gellért Magánklinika Budapest", 10, LatLng(47.707550253211785, 18.613309288842167))
val hospitalTiflis = Hospital("German Hospital", 11, LatLng(41.84625652975446, 44.473796144840954))
val hospitalChisinau = Hospital("Medpark International Hospital", 12, LatLng(46.861595502936744, 28.914024252787808))
val hospitalAtenas = Hospital("Koutanitis, Nik. B., Galinos Polyiatreia S.A.", 13, LatLng(37.96059088783629, 23.737585169307067))
val hospitalErevan = Hospital("Heratsi Hospital Complex", 14, LatLng(40.48659368546972, 44.008989670419645))
val hospitalAnkara = Hospital("Memorial Ankara Hastanesi", 15, LatLng(40.077413596198284, 31.826482715130485))


//Europa del Oeste


val hospitalLisboa = Hospital("Centro Hospitalar Universitário de Lisboa Central - Hospital de São José", 0, LatLng(38.743740845318676, -9.148935812833871))
val hospitalMadrid = Hospital("Hospital de La Princesa", 1, LatLng(41.13111378757351, -3.7657087665523745))
val hospitalParis = Hospital("Lariboisière Hospital AP-HP", 2, LatLng(49.0242468937122, 2.236030216820476))
val hospitalBerna = Hospital("Inselspital", 3, LatLng(46.94756056987751, 7.424605553960608))
val hospitalViena = Hospital("Hospital general de Viena", 4, LatLng(50.65914784863094, 14.827293706081425))
val hospitalBruselas = Hospital("Instituto Jules Bordet", 5, LatLng(50.833507672949644, 4.347087325361968))
val hospitalAmsterdam = Hospital("OLVG, location Spuistraat", 6, LatLng(52.37118763534204, 4.889372025419725))
val hospitalRoma = Hospital("Rome American Hospital", 7, LatLng(41.90003220640084, 12.602381055745447))
val hospitalLuxemburgo = Hospital("Hospital materno Gran Duquesa Charlotte", 8, LatLng(49.617597294338694, 6.098589640663677))
val hospitalAndorraLaVella = Hospital("Nostra Senyora de Meritxell Hospital", 9, LatLng(42.51145937049988, 1.5336092980918983))
val hospitalLondres = Hospital("Royal London Hospital", 10, LatLng(51.517508455928564, -0.06000350159278706))
val hospitalDublin = Hospital("Hospital materno nacional de Dublín", 11, LatLng(53.33990050330571, -6.245567788033456))

//Ejemplo Dijkstra 1

val hospitalPalestina = Hospital("Shifa Hospital",0, LatLng(31.564210962948277, 34.44809650988595))
val hospitalImperioRomano = Hospital("Grand Hospital Roma",1, LatLng(19.418261376406427, -99.1704821023633))
val hospitalRodesia = Hospital("Pairenyatwa", 2, LatLng(-17.811837538502612, 31.04270510913205))
val hospitalZanzibar = Hospital("Tazakhtaa Global Hospital",3, LatLng(-6.166052793163823, 39.19069436868268))
val hospitalVietnamDelSur = Hospital("FV Hospital", 4, LatLng(10.732692633128533, 106.7180550772955))

//Ejemplo Dijkstra 2

val hospitalImperioThalia = Hospital("County General Hospital", 0, LatLng(40.75802344020216, -73.98557478875968))
val hospitalDinastiaKoteron = Hospital("Centro Médico Mercy West", 1, LatLng(40.78128487107749, -73.96650307341754))
val hospitalImperioTribbitus = Hospital("Hospital Universitario Princeton-Plainsboro", 2, LatLng(40.79188495063495, -73.810441720491))
val hospitalDinastiaCriten = Hospital("Sacred Heart Hospital", 3, LatLng(41.02016133740026, -73.75949456689405))
val hospitalDinastiaTeonen = Hospital("hospital de la Ciudad de los Vientos", 4, LatLng(40.777040822201336, -73.87391225807627))

//Ejemplo Dijkstra 3

val hospitalRodemy = Hospital("Hospital Rodemy de las islas de Rodemy", 0, LatLng(12.497454737417018, -162.01089862954507))
val hospitalAtlantis = Hospital("Hospital de la plaza de Mort", 1, LatLng(0.9409548998617143, -161.95010904690577))
val hospitalCiudadelaOlimpica = Hospital("Esquina con Hopital de Javier de la Ciudadela Olimpica", 2, LatLng(-30.735682185677064, -147.93155426594748))
val hospitalCiudadGriega = Hospital("Arquimedes Hospital", 3, LatLng(-39.557833471920844, 74.87117299253617))
val hospitalCrisa = Hospital("Crisa el Hospital de la ciudad", 4, LatLng(40.97694495219145, 50.184302678712406))


//Ejemplo Dijkstra 4

val hospitalDeSanta = Hospital("Hospital de Santa", 0, LatLng(81.70612239996431, -39.87678026623552))
val hospitalHielolandia = Hospital("Hospital de la Universidad de Hielolandia", 1, LatLng(64.13848889084795, -21.927358229883087))
val hospitalArea51 = Hospital("Hospital Confidencial del Area 51", 2, LatLng(37.24197265583247, -115.81914602376035))
val hospitalIlluminati = Hospital("Hospital Illuminati", 3, LatLng(38.87098454458299, -77.05595882683645))
val hospitalCurazao = Hospital("Capriles Kliniek", 4, LatLng(12.141705942030509, -68.90738459157507))

//Ejemplo FloydWarshall 1

val hospitalUnionSovietica = Hospital("Klinichna Likarnya",0, LatLng(50.3624078086145, 30.48940685786275))
val hospitalCoreaDelNorte = Hospital("Ponghwa Clinic",1, LatLng(39.03502295678675, 125.74258531072718))
val hospitalPemba = Hospital("International SOS Pemba Clinic",2, LatLng(-12.961983936283902, 40.5267158110581))
val hospitalMozambique = Hospital("Hospital Central de Maputo", 3, LatLng(-25.47402616456421, 32.82963776125137))
val hospitalDarfur = Hospital("Kabkabaya Hospital", 4, LatLng(13.644477233261153, 24.089446353394038))

//Ejemplo FloydWarshall 2

val hospitalIrania = Hospital("Royal United Hospital", 0, LatLng(51.50082936078128, -0.12462540187787866))
val hospitalLotus = Hospital("Brookwood Medical Center", 1, LatLng(52.22721962628924, -0.7416145812341696))
val hospitalUgevaria = Hospital("California Hospital Medical Center", 2, LatLng(51.51432871002175, -0.17097384717860972))
val hospitalDinada = Hospital("Hospital los Arcos", 3, LatLng(51.08869932432383, -2.513509277373508))
val hospitalAtoseo = Hospital("American British Cowdray Hospital", 4, LatLng(51.493395703313524, -2.2289097779405473))


//Ejemplo FloydWarshall 3

val hospitalSlugGiant = Hospital("The Hospital by Marco", 0, LatLng(14.771101388182156, -85.51882560372941))
val hospitalSlugFlyer = Hospital("Eri Hospital", 1, LatLng(68.20275964643804, -96.41726366903882))
val hospitalLemor = Hospital("Tarma Hospital the yellow", 2, LatLng(51.72460541749809, -41.22194830602026))
val hospitalServil = Hospital("Hospital Fio the health", 3, LatLng(69.09854407629268, 1.668678918745718))
val hospitalAstrolug = Hospital("Hyakutaro Ichimonji del Tenkiu", 4, LatLng(28.146055049368446, -1.4953837453763636))

//Ejemplo Floyd Warshall 4

val hospitalTriangulo = Hospital("Hospital Acuatico del Triangulo de las Bermudas", 0, LatLng(25.000366535858547, -71.00020465819617))
val hospitalUganda = Hospital("Nakasongola Health Center", 1, LatLng(1.4942002601822486, 32.473963167355734))
val hospitalEgipto = Hospital("El Farafra Central Hospital", 2, LatLng(27.154153744366663, 27.98048638195734))
val hospitalSuiza = Hospital("Ärztezentrum Brienz", 3, LatLng(46.76885540609993, 8.031856410365522))
val hospitalBolivia = Hospital("Clinica Talentum", 4, LatLng(-16.561290088491734, -68.0962472369817))










val latamHospitals: HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalSucre,
    1 to hospitalAsuncion,
    2 to hospitalMontevideo,
    3 to hospitalBrasilia,
    4 to hospitalBuenosAires,
    5 to hospitalSantiago,
    6 to hospitalLima,
    7 to hospitalQuito,
    8 to hospitalBogota,
    9 to hospitalCaracas,
    10 to hospitalGeorgetown,
    11 to hospitalParamaribo,
    12 to hospitalCayena
)

val eastEuropeHospital:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalMoscu,
    1 to hospitalHelsinki,
    2 to hospitalOslo,
    3 to hospitalEstocolmo,
    4 to hospitalMinsk,
    5 to hospitalVarsovia,
    6 to hospitalBerlin,
    7 to hospitalPraga,
    8 to hospitalBratislava,
    9 to hospitalKiev,
    10 to hospitalBudapest,
    11 to hospitalTiflis,
    12 to hospitalChisinau,
    13 to hospitalAtenas,
    14 to hospitalErevan,
    15 to hospitalAnkara
)

val westEuropeHospital:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalLisboa,
    1 to hospitalMadrid,
    2 to hospitalParis,
    3 to hospitalBerna,
    4 to hospitalViena,
    5 to hospitalBruselas,
    6 to hospitalAmsterdam,
    7 to hospitalRoma,
    8 to hospitalLuxemburgo,
    9 to hospitalAndorraLaVella,
    10 to hospitalLondres,
    11 to hospitalDublin
)

val exampleHospital1Dijkstra:HashMap<Int, Hospital> = hashMapOf(

    0 to hospitalPalestina,
    1 to hospitalImperioRomano,
    2 to hospitalRodesia,
    3 to hospitalZanzibar,
    4 to hospitalVietnamDelSur


)

val exampleHospital2Dijkstra:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalImperioThalia,
    1 to hospitalDinastiaKoteron,
    2 to hospitalImperioTribbitus,
    3 to hospitalDinastiaCriten,
    4 to hospitalDinastiaTeonen
)

val exampleHospital3Dijkstra:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalRodemy,
    1 to hospitalAtlantis,
    2 to hospitalCiudadelaOlimpica,
    3 to hospitalCiudadGriega,
    4 to hospitalCrisa
)

val exampleHospital4Dijkstra:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalDeSanta,
    1 to hospitalHielolandia,
    2 to hospitalArea51,
    3 to hospitalIlluminati,
    4 to hospitalCurazao
)

val exampleHospital1FloydWarshall:HashMap<Int, Hospital> = hashMapOf(

    0 to hospitalUnionSovietica,
    1 to hospitalCoreaDelNorte,
    2 to hospitalPemba,
    3 to hospitalMozambique,
    4 to hospitalDarfur



)

val exampleHospital2FloydWarshall:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalIrania,
    1 to hospitalLotus,
    2 to hospitalUgevaria,
    3 to hospitalDinada,
    4 to hospitalAtoseo
)

val exampleHospital3FloydWarshall:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalSlugGiant,
    1 to hospitalSlugFlyer,
    2 to hospitalLemor,
    3 to hospitalServil,
    4 to hospitalAstrolug
)

val exampleHospital4FloydWarshall:HashMap<Int, Hospital> = hashMapOf(
    0 to hospitalTriangulo,
    1 to hospitalUganda,
    2 to hospitalEgipto,
    3 to hospitalSuiza,
    4 to hospitalBolivia
)




